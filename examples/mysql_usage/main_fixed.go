...
// directMySQLExampleFixed 直接使用 MySQL 客户端 (修复版本)
func directMySQLExampleFixed() {
	// 配置 MySQL 连接
	config := mysql.Config{
		Host:            "localhost",
		Port:            3306,
...
// factoryExampleFixed 使用工厂模式创建 MySQL 客户端 (修复版本)
func factoryExampleFixed() {
	// 配置数据库连接
	config := db.Config{
		Type: db.TypeMySQL,
		MySQL: mysql.Config{
...
// ormExampleFixed 使用 ORM 进行数据库操作 (修复版本)
func ormExampleFixed() {
	// 配置 MySQL 连接
	config := mysql.Config{
		Host:            "localhost",
		Port:            3306,
...
// transactionExampleFixed 使用事务管理 (修复版本)
func transactionExampleFixed() {
	// 配置 MySQL 连接
	config := mysql.Config{
		Host:            "localhost",
		Port:            3306,
	...
	err = ormInstance.Transaction(ctx, func(txORM *orm.ORM) error {
		...
		if err != nil {
			return fmt.Errorf("获取用户1 ID失败: %w", err)
		}
		...
		if err != nil {
			return fmt.Errorf("获取用户2 ID失败: %w", err)
		}
...
// poolExampleFixed 使用连接池 (修复版本)
func poolExampleFixed() {
	// 配置多个 MySQL 连接
