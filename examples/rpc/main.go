...
func main() {
	...
	degradationConfig := rpc.DefaultDegradationConfig()
	degradationConfig.AutoDetect = true
	degradationConfig.ErrorRateThreshold = 60
	degradationConfig.LatencyThreshold = 3 * time.Second
	degradationManager, err := rpc.NewDegradationManager(degradationConfig)
	if err != nil {
		log.Fatalf("Failed to create degradation manager: %v", err)
	}
	manager.SetDegradationManager(degradationManager)

	// Configure rate limiter manager
	rateLimiterManager := ratelimiter.NewManager(ratelimiter.DefaultConfig())
	if err := rateLimiterManager.AddConfig("default", ratelimiter.Config{
		Strategy: ratelimiter.StrategyTokenBucket,
		Rate:     50,
		Burst:    20,
		Window:   time.Second,
	}); err != nil {
		log.Fatalf("Failed to setup default rate limiter: %v", err)
	}
	if err := rateLimiterManager.AddConfig("calculator.add", ratelimiter.Config{
		Strategy: ratelimiter.StrategyTokenBucket,
		Rate:     10,
		Burst:    5,
		Window:   time.Second,
	}); err != nil {
		log.Fatalf("Failed to setup calculator.add rate limiter: %v", err)
	}
	manager.SetRateLimiterManager(rateLimiterManager)
